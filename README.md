...
**Live:** page at https://ewanrdbradley-rgb.github.io/jeanihealth/memberfeedback/ (repo `jeanihealth`, page in `survey/memberfeedback/`, published by GitHub Actions on every push to `main`; the site root forwards to it). Answers go to the "Jeani onboarding feedback" Google Sheet via its bound Apps Script. Ready-to-paste email block with the live URL filled in: `email/onboarding-snippet.ready.html` (and `.ready.txt`), regenerated by `scripts/build_email.sh`.
...
3. The page is at `https://ewanrdbradley-rgb.github.io/jeanihealth/memberfeedback/`. For a jeanihealth.com address instead, add a CNAME record pointing at `ewanrdbradley-rgb.github.io` and set the custom domain in the same Pages settings screen.
